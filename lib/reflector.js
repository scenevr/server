...
// Get the xml of the documient for saving
Reflector.prototype.getXML = function () {
  var result = ['<scene>'];

  this.scene.childNodes.forEach(function (el) {
    if (el.nodeName === 'player') {
      return;
    }

    // todo: Add some code here to prevent script generated stuff from being
    //   saved out?
    // if ((!el.getPrivateAttribute) || el.getPrivateAttribute('startIndex')) {
    // }
    result.push(el.outerHTML.replace(/uuid=".+?" {0,1}/g, ''));
  });
